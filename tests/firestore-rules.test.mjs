...
test('escalada de rol: un miembro NO puede cambiar su propio rol', async () => {
...
test('borrado de paciente: enfermería NO puede borrar', async () => {
  await assertFails(deleteDoc(doc(dbA(), `hospitals/${HOSP_A}/months/${MES}/patients/p1`)));
});

test('borrado de paciente: admin SÍ puede borrar', async () => {
  await assertSucceeds(deleteDoc(doc(dbAdmin(), `hospitals/${HOSP_A}/months/${MES}/patients/p1`)));
});
